  ...
  const handleGoogleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Errore",
          description: data.message || "Errore durante il login",
          variant: "destructive",
        });
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-chart-2/5 flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl">Accedi a FantaSerieA</CardTitle>
          <CardDescription>
            Inserisci la tua email Google per iniziare a giocare
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="text-center text-sm text-muted-foreground">
            <p>Solo email Google (Gmail, Google Mail, etc.) sono accettate.</p>
            <p>Non hai un account? Inserisci la tua email Google per creare un account.</p>
          </div>
        </CardContent>
      </Card>
    </div>
